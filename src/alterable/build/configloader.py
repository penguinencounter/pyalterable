...
class _PipelineValidator(MapValidator):
    ...
    def validate(chunk: YAMLChunk):
        ...
        Map(_PipelineValidator.BASE_VALIDATOR | _PipelineValidator.OPTIONALS).validate(chunk)
        mode: str | None = None
        ...
        if len(rules) == 0:
            context.append(f"info: Pipeline target {mode!r} requires no additional properties")
        else:
            props = ', '.join(map(lambda x: f'{x!r}', rules.keys()))
            context.append(f"info: Pipeline target {mode!r} requires {len(rules)} additional properties: {props}")
...
def load(conf_path: PathLike) -> YAML:
    ...
    try:
        ...
        if detail_e.problem == "found a blank string":
            err_ctx.append(["hint: if you meant an empty sequence or mapping,\n"
            ...
            keyname = re.search(r"unexpected key not in schema ['\"](.*)['\"]$", detail_e.problem)
            if keyname is not None:
                err_ctx.append([f"hint: try removing {keyname.group(1)!r} and its contents"])
        if len(context) > 0:
            err_ctx.append(context)
        all_messages: list[str] = [f"[bold red]{detail_e.context}: {detail_e.problem}[/]",
                                   f"[red]{detail_e.context}:[/]", f"{escape(str(detail_e.context_mark))}",
                                   f"[red]{detail_e.problem}:[/]", f"{escape(str(detail_e.problem_mark))}"]
